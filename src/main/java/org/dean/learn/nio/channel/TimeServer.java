...
/**
 * 代码示例 3-9 RFC 868 时间服务器
 * Provide RFC 868 time service (http://www.ietf.org/rfc/rfc868.txt).
 * This code implements an RFC 868 listener to provide time service.
 * The defined port for time service is 37. On most unix system, root
 * privilege is required to bind to ports below 1024. You can either
 * run this code as root or provide another port number on the command
 * line. Use "-p port#" with TimeClient if you choose an alternate port.
 *
 * Note : The familiar rdate command on linux will probably not work
 * with this server. Most versions of rdate use TCP rather than UDP to
 * request the time.
 *
 * Created by Dean on 2016/3/28.
 */
